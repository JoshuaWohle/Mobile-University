...
public class DatabaseController extends ListActivity implements OnClickListener {
	...
	public void populateList(ArrayAdapter<DBComments> adapter2) {
		...
		if (user != null && Session.getCurrentSelectedCourse() != null) {
				...
				if (f.isFile()) {
					...
					if (i < adapter2.getCount()) {
						if (f.getName().toString()
								.compareTo(adapter2.getItem(i).toString()) == 0) {
						} else {
			...
			Toast.makeText(getApplicationContext(),
					"There are no files downloaded", Toast.LENGTH_LONG).show();
	...
	public void onClick(View view) {
		...
		case R.id.add:
			...
			if (user != null && Session.getCurrentSelectedCourse() != null) {
				...
				for (File f : moodle.listFiles()) {
					if (f.isFile()) {
						...
						if (i < adapter.getCount()) {
							if (f.getName().toString()
									.compareTo(adapter.getItem(i).toString()) == 0) {
							} else {
								comment = datasource.createComment(name);
								adapter.add(comment);
							}
				...
				Toast.makeText(getApplicationContext(),
						"There are no files downloaded", Toast.LENGTH_LONG)
						.show();
	 ...
	 * position, long id) {
	 * 
	 * if (file != null) { MimeTypeMap myMime = MimeTypeMap.getSingleton();
	 * Intent newIntent = new Intent(android.content.Intent.ACTION_VIEW);
	 * 
	 * //Intent newIntent = new Intent(Intent.ACTION_VIEW); String mimeType =
	 * myMime.getMimeTypeFromExtension(fileExt(file.toString()).substring(1));
	 * newIntent.setDataAndType(Uri.fromFile(file),mimeType);
	 * newIntent.setFlags(newIntent.FLAG_ACTIVITY_NEW_TASK); try {
	 * CourseContentView.this.startActivity(newIntent); } catch
	 * (android.content.ActivityNotFoundException e) { Log.e("MIME Error",
	 * e.toString()+" default program for this filetype not found"); // Raise on
	 * activity not found Toast.makeText(CourseContentView.this,
	 * "A suitable Application to access the file " + mimeType +" not found.",
	 * 4000).show(); } } }
	 */
